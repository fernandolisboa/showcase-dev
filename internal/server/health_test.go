...
func TestRouterServesHealthz(t *testing.T) {
	h := New(slog.New(slog.NewTextHandler(io.Discard, nil)), config.Config{}, nil, nil, nil, nil)
...
func TestReadyz(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			h := New(slog.New(slog.NewTextHandler(io.Discard, nil)), config.Config{}, nil, tc.check, nil, nil)
			rec := httptest.NewRecorder()
